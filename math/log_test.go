...
func (l *testLentzer) Lentz() (f, Δ, C, D, eps *decimal.Big) {
	...
	C = new(decimal.Big)                    // C
	D = new(decimal.Big)                    // D
	eps = decimal.New(1, precision(l.recv)) // eps

	const n = 1
	f.Context.Precision = precision(l.recv) * n
	C.Context.Precision = precision(l.recv) * n
	D.Context.Precision = precision(l.recv) * n
	return
}
...
func benchmarkAlg(b *testing.B, g func(z, x *decimal.Big) *decimal.Big, prec, iters int) {
	lb := new(decimal.Big)
	lb.Context.Precision = prec
